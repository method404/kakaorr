...
export function SeriesSelectionModal({
  ...
  const labels = {
    ...
    addUnavailable:
      ...
      (locale === "ko"
        ? "이 소스의 저장 기능은 아직 준비 중입니다."
        : "Saving for this source is not ready yet."),
  };
  ...
  const handleAdd = () => {
    ...
    if (item.addDisabledReason) {
      setError(labels.addUnavailable);
      return;
    }
      ...
      >
        ...
        <div className="series-modal-content">
          ...
          <div className="series-modal-heading">
            ...
            {item.addDisabledReason ? (
              <p className="series-modal-error">{labels.addUnavailable}</p>
            ) : error ? (
              <p className="series-modal-error">{error}</p>
            ) : null}
          </div>
        </div>
        ...
        <div className="series-modal-footer">
          <button
            ...
            onClick={handleAdd}
            disabled={item.titleId === null || isPending || Boolean(item.addDisabledReason)}
          >
